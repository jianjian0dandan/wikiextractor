...
def main():
    ...
    try:
        args_bytes = args.bytes
    except:
        args_bytes = "100M"

    try:
        power = 'kmg'.find(args_bytes[-1].lower()) + 1
        file_size = int(args_bytes[:-1]) * 1024 ** power
        if file_size < minFileSize:
            raise ValueError()
    except ValueError:
        logging.error('Insufficient or invalid size: %s', args_bytes)
        return

    try:
        args_namespaces = args.namespaces
    except:
        args_namespaces = ""

    if args_namespaces:
        options.acceptedNamespaces = set(args_namespaces.split(','))

    # ignoredTags and discardElemets have default values already supplied, if passed in the defaults are overwritten
    try:
        args_ignored_tags = args.ignored_tags
    except:
        args_ignored_tags = ""
    if args_ignored_tags:
        ignoredTags = set(args_ignored_tags.split(','))
    else:
        ignoredTags = [
    ...
    try:
        args_discard_elements = args.discard_elements
    except:
        args_discard_elements = ""
    if args_discard_elements:
        options.discardElements = set(args_discard_elements.split(','))
    ...
    try:
        args_article = args.article
    except:
        args_article = False

    try:
        args_templates = args.templates
    except:
        args_templates = False

    if args_article:
        if args_templates:
            if os.path.exists(args_templates):
                with open(args_templates) as file:
                    load_templates(file)
    ...
    try:
        ...
        args_processes = default_process_count

    process_dump(input_file, args_templates, output_path, file_size,
                 args_compress, args_processes)
